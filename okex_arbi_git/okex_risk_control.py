...
# https://www.okex.com/academy/zh/option-margin-cn/
class risk_control:

    def __init__(self, api_key: str, secret_key: str, passphrase: str, flag: str, riskcontrol_percentage=0.3):
        self.api_key = api_key
        self.secret_key = secret_key
        self.passphrase = passphrase
        self.flag = flag
        self.risk_control_percentage = riskcontrol_percentage

    def commission(self):
        account = ccxt.okex({'apiKey': self.api_key,
                             'secret': self.secret_key,
                             'password': self.passphrase})
        account.set_sandbox_mode(self.flag)
        lvl = account.private_get_account_config().get('data')[0].get('level')  # 获取手续费率

        comm_swap = {'Lv1': [0.00020, 0.00050],
                     'Lv2': [0.00018, 0.00045],
                     'Lv3': [0.00017, 0.00040],
                     'Lv4': [0.00016, 0.00035],
                     'Lv5': [0.00015, 0.00030]}
        comm_c2c = {'Lv1': [0.00080, 0.00100],
                    'Lv2': [0.00075, 0.00095],
                    'Lv3': [0.00070, 0.00090],
                    'Lv4': [0.00065, 0.00085],
                    'Lv5': [0.00060, 0.00080]}
        return {'comm_c2c': comm_c2c.get(lvl), 'comm_swap': comm_swap.get(lvl)}

    def pos_percentage(self, swap_price, liqPx, position='short'):
        now_px = float(swap_price)
        liqPx = float(liqPx)
        if position == 'short':
            r_diff = (liqPx - now_px) / liqPx
        elif position == 'long':
            r_diff = (now_px - liqPx) / liqPx
        if r_diff <= 0.03:
            return self.risk_control_percentage
        else:
            return 0

    def dual_trade(self, swap_amount, spot_amount, swap_price, spot_price, ccy_list, spot_inst_id, swap_inst_id):
        # SWAP, sz为数量，sz = 10对应0.1个BTC
        # SPOT 需要ccy，sz = 0.1对应0.1个BTC
        sz_spot = str(spot_amount)
        sz_swap = str(int(swap_amount / float(ccy_list[0])))
        account = ccxt.okex({'apiKey': self.api_key,
                             'secret': self.secret_key,
                             'password': self.passphrase})
        account.set_sandbox_mode(self.flag)
        buy_order = account.create_limit_sell_order(spot_inst_id, sz_spot, spot_price,
                                                    params={'tdMode': 'cash'})
        sell_order = account.create_limit_buy_order(swap_inst_id, sz_swap,
                                                    swap_price, params={'tdMode': 'cross'})

        return {'buy_order': buy_order,
                'sell_order': sell_order}

    def upside_risk(self, liqPx: str, swap_amount: float, spot_amount: float, swap_price: str, spot_price: str,
                    order_list, ccy_list: list, spot_instId, swap_instId, position='short'):
        if not order_list:
            per = self.pos_percentage(swap_price, liqPx, position)
            if per == 0:
                return
            elif per == self.risk_control_percentage:
                response = self.dual_trade(swap_amount, spot_amount,
                                           swap_price, spot_price, ccy_list, spot_instId, swap_instId)
                return response

    def downside_risk(self, rate, swap_amount, spot_amount, swap_lever, swap_price, spot_price, order_list,
                      ccy_list, spot_instId, swap_instId, tolerance=0):
        # 交易API端口，交易货币ID如'BTC-USDT-SWAP'，费率,关仓费，开仓费，佣金等其他交易费用，宽容度用来调整费率和费用的差额
        # spot_amount 是spot的比特币个数, swap_amount 是swap的比特币个数
        # 完全对冲情况下， spot_amount * spot_lever = swap_amount * swap_lever
        if rate <= 0:
            if not order_list:
                coms = self.commission()
                cost_hold = abs(rate) * swap_amount * swap_lever  # 持有成本
                cost_close = 2 * swap_amount * coms['comm_swap'][1] + 2 * spot_amount * coms['comm_c2c'][1]  # 交易成本
                if float(cost_hold) / float(cost_close) > 1 + tolerance:
                    response = self.dual_trade(swap_amount, spot_amount, swap_price, spot_price, ccy_list, spot_instId,
                                               swap_instId)
                    return response
...
if __name__ == '__main__':
    ...
    # 下单  Place Order
    a = risk_control(api_key, secret_key, passphrase, flag)
    print(a.pos_percentage(50000, 51000))
    print(a.commission())
'''    
    #result = tradeAPI.place_order(instId='BTC-USDT-SWAP', tdMode='cross', side='sell', ccy='BTC',ordType='market', sz='100')
    #print(result)

    #result = tradeAPI.place_order(instId='BTC-USDT', ccy = 'USDT',tdMode='cross', side='buy',ordType='market', sz='1000')    
    #result = tradeAPI.place_order(instId='BTC-USDT-SWAP', tdMode= 'cross',side= 'sell',ordType= 'market', sz= '4')
